...
const ALLOWANCE = 0.95; // 95% allowance for Stableford
...
export default function ScoreClient() {
  ...
  function getCourseHandicap(handicapIndex: number | null | undefined, teeRow: TeeRow | null): number | null {
    if (handicapIndex == null || !teeRow) return null;
    const ch = handicapIndex * (teeRow.slope_rating / 113) + (teeRow.course_rating - teeRow.course_par);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: teeRow } = await supabase
        .from("tees")
        .select("id, tee_name, par, stroke_index, yardage, metres, course_rating, slope_rating, course_par")
  ...
  function setStroke(playerId: string, value: string) {
    ...
    const safe = Number.isFinite(n as number) && (n as number) > 0 ? (n as number) : undefined;
    setScores((prev) => ({
      ...prev,
      [playerId]: {
        ...(prev[playerId] ?? {}),
  ...
  async function handleNext() { await saveHole(); nextHole(); }
  async function handlePrev() { await saveHole(); prevHole(); }
  async function handleFinish() { await saveHole(); setShowFinishConfirm(false); router.push(`/leaderboard?competition_id=${competitionId}`); }
  ...
  if (!competitionId) {
    return (
      <div className="p-4 text-red-600">Missing competition id. Please start from the Players page.</div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-white rounded-lg shadow p-5 text-center border">
        <h2 className="text-4xl font-extrabold uppercase tracking-wide">{holeTitle}</h2>
        {holeSI != null && (<p className="text-lg font-semibold mt-1">Stroke Index: {holeSI}</p>)}
        {tee && (<p className="mt-2 text-gray-700 font-semibold uppercase">{tee.tee_name} TEE</p>)}
        {(holeYards != null && holeMetres != null) && (
          <p className="text-sm text-gray-600 mt-1">{holeYards} yds / {holeMetres} m</p>
        )}
      </div>

      {/* Prev / Next / Finish */}
      <div className="flex items-center justify-between bg-white p-3 rounded shadow">
        <button className="px-3 py-2 bg-gray-200 rounded" onClick={handlePrev}>Prev</button>
        <div />
        {currentHole < 18 ? (
          <button onClick={handleNext} className="px-3 py-2 bg-gray-800 text-white rounded">Next</button>
        ) : (
          <button onClick={() => setShowFinishConfirm(true)} className="px-3 py-2 bg-green-600 text-white rounded">Finish</button>
        )}
      </div>

      {/* Leaderboard Links */}
      <div className="flex gap-2">
        <Link href={`/leaderboard?competition_id=${competitionId}`} target="_blank" className="px-3 py-2 bg-blue-600 text-white rounded">Open Leaderboard</Link>
        <Link href={`/leaderboard-tv?competition_id=${competitionId}`} target="_blank" className="px-3 py-2 bg-gray-800 text-white rounded">Open TV Leaderboard</Link>
      </div>
      ...
      <div className="space-y-2">
        {players.map((p) => {
          ...
          const ptsColor =
            pts >= 4 ? "text-purple-600" :
            pts === 3 ? "text-green-600" :
            pts === 2 ? "text-blue-600" :
            pts === 1 ? "text-orange-600" : "text-red-600";
          const cumulative = getCumulativePointsUpTo(p.id, currentHole, tee, scores, playingHcp);
          ...
          return (
            <div key={p.id} className="bg-white p-3 rounded shadow space-y-1">
              ...
              <div className="grid grid-cols-12 gap-2 items-center">
                <div className="col-span-7 font-medium">
                  ...
                  <div className="text-xs text-gray-600">
                    HI {p.handicap_index.toFixed(1)} • CH {courseHcp ?? "-"} • PH {courseHcp == null ? "-" : playingHcp}
                  </div>
                </div>
                <div className="col-span-5 flex items-center justify-end gap-2">
                  <button className="h-10 w-10 rounded bg-gray-200 text-xl leading-none" onClick={() => adjustStroke(p.id, -1)}>–</button>
                  <input
              ...
              <div className="text-sm flex justify-between px-1">
                ...
                <span className="text-gray-600">(Receives {shots} shot{shots === 1 ? "" : "s"} on this hole)</span>
              </div>
      ...
      {showFinishConfirm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 rounded shadow-xl space-y-4 max-w-sm text-center">
            ...
            <p className="text-gray-700">Are you sure you want to finish the round? You can still go back and adjust scores if needed.</p>
            <div className="flex justify-center gap-4">
              <button onClick={() => setShowFinishConfirm(false)} className="px-4 py-2 bg-gray-200 rounded">Cancel</button>
              <button onClick={handleFinish} className="px-4 py-2 bg-green-600 text-white rounded">Yes, Finish</button>
            </div>
          </div>
        </div>
      )}
