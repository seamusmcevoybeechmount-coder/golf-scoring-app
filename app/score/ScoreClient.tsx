...
export default function ScoreClient() {
  ...
  // Navigation
  function nextHole() { setCurrentHole((h) => (h % 18) + 1); }
  function prevHole() { setCurrentHole((h) => ((h - 2 + 18) % 18) + 1); }
  ...
  async function handleFinish() { await saveHole(); setShowFinishConfirm(false); window.location.href = `/leaderboard?competition_id=${competitionId}`; }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="space-y-2">
        {players.map((p) => {
          ...
          const cumulative = getCumulativePointsUpTo(p.id, currentHole, tee, scores, p.playing_handicap);
            ...
            <div key={p.id} className="bg-white p-3 rounded shadow space-y-1">
              ...
              <div className="grid grid-cols-12 gap-2 items-center">
                ...
                <div className="col-span-5 flex items-center justify-end gap-2">
                  ...
                  <input className="w-20 text-center p-2 border rounded" inputMode="numeric" value={strokes.toString()} onChange={(e) => setStroke(p.id, e.target.value)} />
                  <button className="h-10 w-10 rounded bg-gray-200 text-xl leading-none" onClick={() => adjustStroke(p.id, +1)}>+</button>
                </div>
              </div>
