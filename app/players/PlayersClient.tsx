...
export default function PlayersClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setTees(data ?? []);
      // Preselect first tee for each player row
      if (data && data.length) {
        setPlayers((prev) =>
  ...
  const canSubmit = useMemo(() => {
    ...
    return players.some((p) => p.name.trim()) &&
           players.every((p) => !p.name || p.teeId); // if a name is given, a tee must be chosen
  ...
  async function handleStart() {
    ...
    const cleaned = players
      ...
      .filter(
        (p) => p.name && !Number.isNaN(p.handicap_index) && p.tee_id
      );

    if (cleaned.length === 0) return;

    // 2) Create competition using the FIRST player’s tee just to satisfy schema
    const compRes = await fetch("/api/competitions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        course_id: courseId,
        tee_id: cleaned[0].tee_id, // competition tee (display/default)
        name: null,
      }),
    });
    if (!compRes.ok) {
    ...
    // 3) Add players with *their own* tee_id values
    const addRes = await fetch("/api/players", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        competition_id,
        players: cleaned, // each item has { name, handicap_index, tee_id }
      }),
    });
    if (!addRes.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!courseId && (
        <p className="text-red-600">No course selected. Go back and pick a course.</p>
      )}
